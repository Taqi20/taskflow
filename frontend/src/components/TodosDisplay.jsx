...
function AddTask(props) {
    ...
    return (
        <div
            className={`bg-[#f6f6f6] flex items-center  px-2 w-5/6 mx-auto h-12 rounded-md opacity-80 hover:opacity-100 cursor-text absolute bottom-7 left-0 right-0 ${isInputFocused ? "opacity-100" : "opacity-80"
                }`}
        ...
        >
            {!isInputFocused ? (
                ...
                <img src={circleIcon} alt="" className="h-9 py-2 mr-3" />
            )}
...
function TodoComponent(props) {
    ...
    const { completedTodoStyle, handleOnCheck, handleToggleMarkedImp } = useTodoContext();
        ...
        >
            ...
            >
                {!thisTodo.isCompleted ? (
                    <img
                        src={isHovered ? hoverCheckIcon : circleIcon}
                        alt=""
            ...
            <span
                className={`w-[90%] overflow-hidden whitespace-nowrap overflow-ellipsis ${thisTodo.isCompleted ? completedTodoStyle : ""
                    }`}
            >
                {todoTitle}
            </span>
...
export default function TodosDisplay() {
    ...
    const { todos, setTodos, addTodo, selectTodo, selectedTodo, getTodosData } = useTodoContext();
    ...
    return (
        <div>
            <div id="id" className="bg-[#5c70be] w-full overflow-hidden flex">
                <div
                ...
                >
                    ...
                    <div className="px-3 pt-5 lg:hidden">
                        <img src={listIconWhite} alt="toggle"
                        ...
                        <div className="w-full h-[70%] md:h-[72%] lg:h-[70%] xl:h-4/6 2xl:h-[74%] bg-[#5c70be] overflow-y-auto space-y-1">
                            ...
                            {todos && todos.length !== 0 && todos.map && todos.some((todo) => todo.isCompleted) ? (
                                <div id="completedTask" className="w-full">
                                    <div className="w-5/6 mx-auto mt-4">
                                        ...
                                        >
                                            <img src={dropdownArrow} alt=""
                                                className={`h-5 transition-transform ${isCompletedTaskOpen ? "" : "rotate-180"
                                                    }`} />
                                            <span>Completed</span>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        className={`space-y-1 mt-1 ${isCompletedTaskOpen ? "" : "hidden"
                                            }`}
                                    >
                                        {todos.map((todo, index) => {
                                            if (todo.isCompleted) {
                    ...
                    <div
                        ...
                        className={` ${selectedTodo ? "w-full md:w-[75%] xl:w-[52%]" : "w-0"
                            } duration-300`}
                    >
                        <TodoSidebar />
                    </div>
